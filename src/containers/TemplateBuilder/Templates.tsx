...
import { Button, Header, Icon, Table, Label, Dropdown, Checkbox, Confirm, Input } from "semantic-ui-react";
...
const columns: Columns = [
  {
    ...
    render: ({ template: { code, priority } }) => (
      <>
        <span>{code}</span>
        {priority && <Label content={`Priority: ${priority}`} size="mini" style={{ marginLeft: 5 }} />}
      </>
    ),
  },
  {
      ...
      <Confirm
        ...
        content={
          <div style={{ padding: 10, gap: 10 }} className="flex-column">
            ...
            <p>
              By exporting this template now, you will be committing the current version. To make any further changes,
              you will need to duplicate it and start a new template version.
            </p>
      ...
      <Confirm
        ...
        content={
          <div style={{ padding: 10, gap: 10 }} className="flex-column">
            ...
            <p>Do you want to create a new template version or a whole new template type based on this template?</p>
        ...
        onConfirm={async () => {
          ...
          if (
            ...
            showToast({
              title: selectedType === "template" ? "New template created" : "New template version created",
...
const Templates: React.FC = () => {
  ...
  const renderTemplate = (template: CellPropsTemplate, refetch: () => void, hasChildren = false) => (
    <Table.Row
        ...
        <div key="listContainer" id="list-container" className="outcome-table-container">
          <div className="flex-row-end" style={{ alignItems: "center", gap: 20 }}>
            ...
            <Dropdown
              ...
              onChange={(_, { value }) => {
                updateQuery({
                  categories: (value as string[]).map((cat) => (cat === "" ? "none" : cat)).join(","),
                });
          ...
          <Table sortable stackable selectable>
            {renderHeader()}
            <Table.Body key="body">
              {sortedTemplates.map(({ others, main, totalApplicationCount, numberOfVersions }, rowIndex) => (
...
const filterTemplates = (templates: Templates, categories: string[], hideInactive: boolean) => {
  const categoryFiltered =
    categories.length === 0 ? templates : templates.filter(({ main }) => categories.includes(main.category));
  return hideInactive ? categoryFiltered.filter((template) => template.main.status === "AVAILABLE") : categoryFiltered;
};

const sortTemplates = (templates: Templates, sortColumn: SortColumn | undefined, sortAsc: 1 | -1) => {

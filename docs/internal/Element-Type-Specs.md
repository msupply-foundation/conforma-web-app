...
- **tableWidth** `number | string` (only relevant for **table** view) -- explicitly set the width of the table. If not specified, the width is calculated automatically based on the column content (and the table may overflow its container rather than squeezing columns). (Can be any valid CSS value, e.g. `500`, `"600px"`, `"100%"`)

...
- If there isn't one already, create a new file that just returns the query (using the GraphQL query language).
  ...
  - Use a GUI tool to see the results for your query/mutation to the GraphQL server: `http://localhost:5010/graphiql` (if the server is running on localhost:5010).
